export class Trashstore {
  constructor() {
    this.MICRO_TRASH_MAX = this.microTable[2050]["growth"];
    this.MACRO_TRASH_MAX = this.macroTable[2050]["growth"];
  }

  getMicroTrashFactor(year, mode) {
    if (year <= 2020) {
      return this.microTable[year] / this.MICRO_TRASH_MAX;
    } else if (year <= 2050) {
      return this.microTable[year][mode] / this.MICRO_TRASH_MAX;
    }
  }

  getMacroTrashFactor(year, mode) {
    if (year <= 2020) {
      return this.macroTable[year] / this.MACRO_TRASH_MAX;
    } else if (year <= 2050) {
      return this.macroTable[year][mode] / this.MACRO_TRASH_MAX;
    }
  }
